...
InputCoreSystem::InputCoreSystem()
{
}
...
void InputCoreSystem::CarryCurrentInput()
{
	SDL_memcpy(previous_, current_, sizeof(Keystate) * (Uint16)Key::KeyCount);
}

bool InputCoreSystem::KeyDown(Key key)
{
	return previous_[(Uint16)key] == Keystate::Up &&
		current_[(Uint16)key] == Keystate::Down;
}

bool InputCoreSystem::KeyUp(Key key)
{
	return previous_[(Uint16)key] == Keystate::Down &&
		current_[(Uint16)key] == Keystate::Up;
}

bool InputCoreSystem::KeyPressed(Key key)
{
	return previous_[(Uint16)key] == Keystate::Down &&
		current_[(Uint16)key] == Keystate::Down;
}

void InputCoreSystem::UpdateKey(SDL_Keysym &keysym, Keystate state)
{
	switch (keysym.mod)
	{
