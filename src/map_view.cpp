...
void MapView::Initialize()
{
	...
	glm::vec2 size_translate = graphics_core_.PixelsToScale(glm::uvec2(map_.tile_width, map_.tile_height));
		...
		for (int y = 0; y < layer.height; ++y)
		{
			for (int x = 0; x < layer.width; ++x)
			{
				...
				if (!rel_texture_path.empty())
				{
					...
					glm::mat4 transform = glm::scale(glm::vec3(graphics_core_.PixelsToScale(glm::uvec2(tile_data.image_width, tile_data.image_height)), 1.0f));
